...
public:
	...
	void OnMouseDown(int x, int y, IMouseMod* pMod)
	{
		...
		if (using_bitmaps && fast_bitmap_resizing)
		{
			...
			{
				IControl* pIControl = GetGUI()->GetControl(i);
				pIControl->Draw(GetGUI());
			}


			// This will handle drawing tmp bitmap on screen while dragging
			draggingDisplay = (LICE_IBitmap*) new LICE_MemBitmap(plugin_width, plugin_height, 0);

			LICE_Copy(draggingDisplay, (LICE_IBitmap*)GetGUI()->GetDrawBitmap());
	...
	void OnMouseUp(int x, int y, IMouseMod* pMod)
	{
		...
		if (using_bitmaps && fast_bitmap_resizing)
		{
			...
			if (draggingDisplay != NULL)
			{
				delete draggingDisplay;
	...
	bool Draw(IGraphics* pGraphics)
	{
		if (mouse_is_down)
		{
			...
			_LICE::LICE_ScaledBlit(GetGUI()->GetDrawBitmap(), draggingDisplay, 0, 0, mouse_x, mouse_y, 0.0f, 0.0f,
				(float)draggingDisplay->getWidth(), (float)draggingDisplay->getHeight(), 1.0f, LICE_BLIT_MODE_COPY | LICE_BLIT_FILTER_BILINEAR);

			//IColor backgroundColor = IColor(255, 50, 25, 25);
